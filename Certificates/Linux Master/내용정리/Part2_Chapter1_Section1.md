...
사용자명 : pw(X) : UID : GID : comment : home dir : login SHELL

###### /etc/shadow : 사용자 패스워드 저장
사용자명 : 암호화된 pw 맨 앞이 알고리즘 : 마지막 변경일 : 최소 사용일, 최대 사용일, 만료 경고일, 유예기간, 만료일

##### /etc/default/useradd
`useradd` 명령어 사용시 기본 설정 값
'''bash
...
##### /etc/login.defs : shadow password suite 설정
- MAIL_DIR : 메일 보관함 위치
- PASS_MAX_DAYS : 사용 가능한 최대일
- PASS_MIN_DAYS : ...
- PASS_MIN_LEN
- PASS_WARN_AGE : 패스워드 만료 전 경고 일수
- UID_MIN, UID_MAX, GID_MIN, GID_MAX
- CREATE_HOME : 홈 DIR 생성 여부
- UMASK : 없으면 022
- USERGROUPS_ENAB : yes 인 경우 userdel을 통해서 사용자 삭제하면 그룹도 삭제됨.(다른 사용자가 없을 시)
- encrypt_method : 암호화

##### /etc/group : 그룹 사용자 관리
그룹명 : pw : GID : 사용자 1, 사용자 2, ...

##### /etc/gshadow
그룹명 : 암호화된 pw : 그룹 관리자 : 사용자1, 사용자2, ...
...
#### ✅ `users`
| 설명       | 현재 로그인 중인 **사용자 이름만** 출력 |
|------------|------------------------------------------|
| 사용 예시  | `users` |
| 출력 예시  | `francis stan steve` |
| 특징       | 단순 사용자명만 보여줌 (tty/시간 등 정보 없음) |

---

#### ✅ `who`
| 설명       | 현재 로그인 사용자에 대한 **상세 정보** 출력 |
|------------|---------------------------------------------|
| 사용 예시  | `who` |
| 출력 예시  |francis tty1 2023-02-02 19:20</br>stan pts/0 2022-01-01 20:20|
**📌 주요 옵션**

| 옵션  | 설명                                      |
|-------|-------------------------------------------|
| `-a`  | 모든 정보를 출력 (기타 옵션 포함)         |
| `-b`  | 마지막 시스템 부팅 시간 표시              |
| `-H`  | 헤더 포함 출력                            |
| `-q`  | 로그인한 사용자 수와 사용자 목록 출력     |

---
#### ✅ `whoami`
| 설명       | 현재 쉘에서 **실행 중인 사용자명** 출력 |
|------------|-------------------------------------------|
| 사용 예시  | `whoami` |
| 출력 예시  | `sophie` |
| 특징       | `id -un` 과 동일 결과 |
...
#### ✅ `w`
| 설명       | 현재 로그인한 사용자들의 **작업 및 상태 정보** 표시 |
|------------|------------------------------------------------------|
| 사용 예시  | `w`, `w sophie` |
| 출력 항목  | 사용자명, tty, 로그인 시간, idle, CPU 사용, 실행 중 명령 등 |
| 특징       | `who` + 사용자 작업 상태까지 포함 |

---

#### ✅ `logname`
| 설명       | 로그인할 때 사용한 **로그인 계정명** 출력 |
|------------|--------------------------------------------|
| 사용 예시  | `logname` |
| 특징       | `whoami` 와 달리 **로그인 세션 기준** |

---

#### ✅ `id`
| 설명       | 사용자 및 그룹 ID 정보 출력 |
|------------|-----------------------------|
| 사용 예시  | `id`, `id steve` |
| 출력 예시  |uid=1000(steve) gid=1000(steve) groups=1000(steve),27(sudo)|
...
| 설명       | 사용자가 속한 **그룹 목록** 출력 |
|------------|-----------------------------------|
| 사용 예시  | `groups`, `groups steve` |
| 출력 예시  | `steve : steve sudo docker` |
| 특징       | 로그인 사용자 또는 지정한 사용자의 그룹 목록을 출력 |
...
| 명령어        | 설명 |
|---------------|------|
| `pwconv`      | `/etc/passwd` → `/etc/shadow` 로 **암호 분리** |
| `pwunconv`    | `/etc/shadow` → `/etc/passwd` 로 **병합** |
| `grpconv`     | `/etc/group` → `/etc/gshadow` 로 **암호 분리** |
| `grpunconv`   | `/etc/gshadow` → `/etc/group` 로 **병합** |
...
| 명령어    | 설명                         |
|-----------|------------------------------|
| `pwck`    | `/etc/passwd`, `/etc/shadow`의 **정합성 검사** |
| `grpck`   | `/etc/group`, `/etc/gshadow`의 **정합성 검사** |
